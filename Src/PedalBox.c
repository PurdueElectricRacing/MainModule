...
void taskPedalBoxMsgHandler(void * params) {
	...
	uint32_t t1min, t1max, t2min, t2max, b1min, b1max, b2min, b2max;   // Declared static for easy init to 0
	...
	while (1)
	{
		...
		if (queued == pdTRUE)
		{
			...
			throttle_avg = (throttle1_cal + throttle2_cal) / 2.0;
			float brake_avg    = (brake1_cal + brake2_cal) / 2.0;

			// APPS Sensors out of range with each other
